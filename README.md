...
report with `valid`, `status`, `error_code`, `normalized_error_code`,
`error_count`, `require_selected`, `failure_category`, `requires_operator`,
`next_command`, and `errors`; local absolute paths in that report are redacted
as `<path>`.
