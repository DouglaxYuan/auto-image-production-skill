...
`login_required`, `concurrency_limited`, `rate_limited`, `network_error`,
`page_load_failed`, `moderation_blocked`, `download_failed`, and `generation_timeout`. This lets automation resume, back off, switch providers,
...
CAPTCHA, login-required sessions, page load, network or download errors, rate limits, and concurrency limits from all
