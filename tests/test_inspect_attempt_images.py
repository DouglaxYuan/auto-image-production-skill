...
class InspectAttemptImagesTest(unittest.TestCase):
    ...
    def test_cli_normalizes_passing_ocr_status_unicode_dashes(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "A-0001-001"
            root.mkdir()
            data = self.valid_manifest(root)
    ...
    def test_cli_normalizes_forbidden_visible_mark_separators(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "A-0001-001"
            root.mkdir()
            data = self.valid_manifest(root)
    ...
    def test_cli_normalizes_forbidden_ocr_text_pattern_separators(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "A-0001-001"
            root.mkdir()
            data = self.valid_manifest(root)
