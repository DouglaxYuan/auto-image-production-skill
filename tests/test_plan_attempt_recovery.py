...
class PlanAttemptRecoveryTest(unittest.TestCase):
    ...
    def test_cli_classifies_browser_control_unavailable_as_retry(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "A-0001-001"
            root.mkdir()
            manifest_path = self.write_manifest(
                root, self.failed_manifest(root, "browser_control_unavailable")
            )
    ...
    def test_cli_classifies_upload_failure_as_retry(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "A-0001-001"
            root.mkdir()
            manifest_path = self.write_manifest(root, self.failed_manifest(root, "upload_failed"))
    ...
    def test_cli_classifies_ocr_text_detected_as_quality_gate_reroute(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "A-0001-001"
            root.mkdir()
            manifest_path = self.write_manifest(root, self.failed_manifest(root, "ocr_text_detected"))
    ...
    def test_cli_classifies_ocr_status_failed_as_quality_gate_reroute(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / "A-0001-001"
            root.mkdir()
            manifest_path = self.write_manifest(root, self.failed_manifest(root, "ocr_status_failed"))
