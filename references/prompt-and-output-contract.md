...
For provider interruptions such as CAPTCHA, quota, network errors, rate limits, or concurrency limits, write a failed attempt instead of abandoning local state:
...
page load, network, download, and timeout failures to `retry`, model rate limits and concurrency pressure to
