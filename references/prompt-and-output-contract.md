...
The validator checks required fields, `candidate_count`, `result_binding`, candidate file paths, candidate path containment inside the attempt directory, candidate task ids, and `selected_path` membership. It does not replace image decoding, OCR, perceptual hashing, or project-specific quality checks.
